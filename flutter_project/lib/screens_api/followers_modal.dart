...
class FollowersModal extends StatefulWidget {
  const FollowersModal({Key? key}) : super(key: key);

  @override
  _FollowersModalState createState() => _FollowersModalState();
}
...
class _FollowersModalState extends State<FollowersModal> {
  ...
  Future<void> _loadFollowers() async {
    ...
    try {
      final response = await lkService.fetchFollowers(skip, limit);
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            if (index < followers.length) {
              ...
              print('Avatar URL (FollowersModal): $avatarUrl'); // Для отладки
              return ListTile(
