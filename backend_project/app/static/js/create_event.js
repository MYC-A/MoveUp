document.addEventListener("DOMContentLoaded", function () {
    ...
    function addMarker(location) {
        ...
        if (existingMarker) {
            ...
            marker.on('dragend', function () {
                updatePolyline();
                updateRoute(); // Обновляем маршрут через API
            });
        }
    ...
    async function buildRoute() {
        // Проверяем, включена ли опция кругового маршрута
        const roundTrip = document.getElementById("roundTrip").checked;

        if (roundTrip && markers.length < 2) {
            alert("Для кругового маршрута добавьте как минимум две точки.");
            return;
        } else if (!roundTrip && markers.length < 2) {
        ...
        if (roundTrip) {
            coordinates.push(coordinates[0]); // Замыкаем маршрут
        }
        ...
        try {
            ...
            if (data.features && data.features.length > 0) {
                ...
                // Добавляем интерактивные точки на маршрут
                addInteractivePoints(route);
            } else {
    ...
    // Привязка точек к дорогам через OpenRouteService
    async function snapToRoads() {
        ...
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json();
                console.error("Ошибка при привязке к дорогам:", errorData);
                return;
            }
            ...
            if (data.features && data.features.length > 0) {
                ...
                markers.forEach((marker, index) => {
                    const newLatLng = L.latLng(snappedCoordinates[index][1], snappedCoordinates[index][0]);
    ...
    const snapToRoadsButton = document.getElementById("snapToRoads");
    if (snapToRoadsButton) {
        snapToRoadsButton.addEventListener("click", snapToRoads);
    } else {
        console.error("Кнопка #snapToRoads не найдена!");
    }
